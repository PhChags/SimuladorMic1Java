...
public class CPU {
        // Lista de registradores da CPU, com valores e nomes iniciais
        private final ObservableList<Register> registers;
        private short ALatch, BLatch; // Latches para valores temporários usados em operações
        private final Mux aMux; // Multiplexador para selecionar entradas de A
        private final Mux mMux; // Multiplexador para manipular valores do MPC
        private final ALU alu; // Unidade Lógica e Aritmética (ALU)
        private final Shifter shifter; // Unidade responsável por deslocar bits
        private final SimpleIntegerProperty MAR; // Registrador de Endereço de Memória (Memory Address Register)
        private final SimpleIntegerProperty MBR; // Registrador de Buffer de Memória (Memory Buffer Register)
        private final SimpleIntegerProperty MPC; // Contador do Microprograma (Microprogram Counter)
        private final int[] controlMemory; // Memória de controle para armazenar microinstruções
        private final SimpleIntegerProperty MIR; // Registrador de Instrução de Microprograma (Microprogram Instruction Register)
        private byte incrementer; // Incrementador do MPC
        private short aDec, bDec, cDec; // Decodificadores para entradas e saídas
        private final MSeqLogic mSeqLogic; // Lógica Sequencial da Memória
        private final SimpleIntegerProperty clock; // Relógio para controle de ciclos
        private final SimpleIntegerProperty clockCounter; // Contador de ciclos de clock
        private boolean memoryReadDone; // Flag para indicar conclusão de leitura de memória
        private final Memoria memory; // Instância de memória para operações de leitura e escrita
    
        public CPU() {
            // Inicializa os registradores com valores padrão
            registers = FXCollections.observableArrayList(
                    new Register("PC", (short) 0),   // Contador de Programa
                    new Register("AC", (short) 0),   // Acumulador
                    new Register("SP", (short) 4095),// Ponteiro de Pilha
                    new Register("IR", (short) 0),   // Registrador de Instrução
                    new Register("TIR", (short) 0),  // Registrador Temporário de Instrução
                    new Register("0", (short) 0),    // Registrador constante zero
                    new Register("+1", (short) 1),   // Registrador constante +1
                    new Register("-1", (short) -1),  // Registrador constante -1
                    new Register("AMASK", (short) 0x0fff), // Máscara de endereço
                    new Register("SMASK", (short) 0x00ff), // Máscara de sinal
                    new Register("A", (short) 0),    // Registrador A
                    new Register("B", (short) 0),    // Registrador B
                    new Register("C", (short) 0),    // Registrador C
                    new Register("D", (short) 0),    // Registrador D
                    new Register("E", (short) 0),    // Registrador E
                    new Register("F", (short) 0)     // Registrador F
            );
            aMux = new Mux();
            mMux = new Mux();
            ALatch = 0;
            ...
            incrementer = 0;
            controlMemory = LeitorArquivos.getControlMemory(); // Carrega a memória de controle a partir de um arquivo
            aDec = 0;
            bDec = 0;
            cDec = 0;
        ...
        // Configura os valores iniciais da CPU
        public void setCPUInitial() {
            setRegistersInitial(); // Configura os registradores com valores iniciais
            ALatch = 0;
            BLatch = 0;
            ...
            clockCounter.set(0);
            memoryReadDone = false;
            memory.setMemoryInitial(); // Inicializa a memória
        }
    
        // Configura os valores iniciais dos registradores
        private void setRegistersInitial() {
            registers.get(0).setValue((short) 0); // PC
            registers.get(1).setValue((short) 0); // AC
            registers.get(2).setValue((short) 4095); // SP
            registers.get(3).setValue((short) 0); // IR
            registers.get(4).setValue((short) 0); // TIR
            // Configuração dos registradores constantes e auxiliares
        ...
        // Demais métodos mantêm as operações básicas da CPU...
